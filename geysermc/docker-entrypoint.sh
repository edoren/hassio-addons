...
if [ -f "$LAST_VERSION_FILE" ]; then
    ...
    echo "Last executed version $file_version build $file_build";
    if [[ $file_version != $response_version ]] || [[ $file_build != $response_build ]]; then
        echo "Newer version found $response_version build $response_build";
...
if [[ $download_latest_version -eq 1 ]]; then
    ...
    mkdir -p /opt/geyser;
    wget -O /opt/geyser/Geyser.jar --content-disposition --quiet "https://download.geysermc.org/v2/projects/geyser/versions/${config_version}/builds/latest/downloads/standalone";
